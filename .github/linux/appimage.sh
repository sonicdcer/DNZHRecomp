...
cp DNZHRecompiled AppDir/usr/bin/
cp -r assets/ AppDir/usr/bin/
cp recompcontrollerdb.txt AppDir/usr/bin/
cp icons/512.png AppDir/DNZHRecompiled.png
cp .github/linux/DNZHRecompiled.desktop AppDir/

"./linuxdeploy-$LINUX_DEPLOY_ARCH.AppImage" --appimage-extract
mv squashfs-root/ deploy
./deploy/AppRun --appdir=AppDir/ -d AppDir/DNZHRecompiled.desktop -i AppDir/DNZHRecompiled.png -e AppDir/usr/bin/DNZHRecompiled --plugin gtk
...
echo '    APP_FOLDER_PATH=$APP_FOLDER_PATH ./DNZHRecompiled' >> AppDir/AppRun
echo 'else' >> AppDir/AppRun
echo '    cd "$this_dir"/usr/bin/' >> AppDir/AppRun
echo '    ./DNZHRecompiled' >> AppDir/AppRun
